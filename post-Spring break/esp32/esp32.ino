...
const float FLOW_FAULT_LPM = 3.0; ////
const int FLOW_FAULT_DELAY_S = 5;   //
                                    //
bool flowLow = false;               //// For fault detection
...
// For fault detection
void updateFlowFault(float flowLpm)
{
  unsigned long now = millis();

  flowLow = (flowLpm < FLOW_FAULT_LPM);
  ...
  if (flowLow)
  {
    ...
    if (secLowFlow >= FLOW_FAULT_DELAY_S)
    {
      flowFault = true;
    }
  }
...
void handleCycleCommand(const String &line)
{
  ...
  if (cycleState == CYCLE_FAULT)
  {
    ...
    {
      Serial.println("{\"ack\":\"CYCLE\",\"err\":\"FAULT_ACTIVE\"}");
    }
...
void faultHandler(bool okTemp, float tempF, bool levelOk, bool lidClosed, bool okTurb)
{
  bool fault = false;

  if (!okTemp || isnan(tempF))  
  {
    fault = true;   // Throws fault if temp is too high or reads invalid value
  }

  if (!levelOk)
  {
    fault = true;   // Throws fault if levels are critically low
  }

  if (flowFault)
  {
    fault = true;   // Throws fault if flow is too low for too long
  }

  if (cycleState == CYCLE_WASHING && !lidClosed)
  {
    fault = true;   // Throws fault if lid is open while device is running
  }

  if (heaterStop)
  {
    fault = true;   // Throws fault if overtemp shutdown occurs
  }

  // We can add something for turbididty but I don't think we should be shutting down the machine for turbidity
...
void loop()
{
  ...
  if (now - lastTick >= 1000) // Once per second
  {
    ...
    faultHandler(okT, tempF, levelOk, lidClosed, okTur);   // Checks for faults before configuring heater
